...
function readLine(l)
   ...
   if state == "seeds" then
      for n,m in l:gmatch("(%d+) (%d+)") do
	 table.insert(seeds, {start=tonumber(n), l=tonumber(m)})
      end
   else
...
for _,seedRng in ipairs(seeds) do
   print(seedRng.start .. ".." .. seedRng.start+seedRng.l)
   for seed = seedRng.start, seedRng.start+seedRng.l-1 do
      local val = seed
      for i = 1,#chain-1 do
